...
export class JscpdServerService {
  ...
  async checkSnippet(request: CheckSnippetRequest): Promise<CheckSnippetResponse> {
    ...
    const snippetDir = join(tmpdir(), 'jscpd-server-snippets');
    if (!existsSync(snippetDir)) {
      mkdirSync(snippetDir, { recursive: true });
    }

    const snippetId = createHash('md5')
      .update(request.code + Date.now())
      .digest('hex');

    let filename = request.filename || `snippet_${snippetId}`;

    if (request.language && !filename.includes('.')) {
      ...
      filename += extensions[request.language.toLowerCase()] || '.txt';
    }

    const snippetPath = join(snippetDir, filename);
    writeFileSync(snippetPath, request.code, 'utf-8');

    const format = request.language || getFormatByFile(snippetPath, this.options.formatsExts);
    ...
    const snippetClones = clones.filter(
      (clone) =>
        clone.duplicationA.sourceId === snippetPath ||
        clone.duplicationB.sourceId === snippetPath
    );

    const duplications: SnippetDuplication[] = snippetClones.map((clone) => {
    ...
    const percentageDuplicated = totalLines > 0 ? Math.round((10000 * duplicatedLines) / totalLines) / 100 : 0;
